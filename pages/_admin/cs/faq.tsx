...
const AdminFaq: NextPage = ({ initialInquiry }: any) => {
	...
	const removeHandler = async (id: string) => {
		try {
			if (await sweetConfirmAlert('Permanently remove this FAQ entry?')) {
				// wire up mutation when backend is ready
			}
		} catch (err: any) {
			sweetErrorHandling(err).then();
	...
	return (
		<Box component="div" className="content">
			<Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 3 }}>
				<Box component="div">
					<Typography sx={{ fontSize: 20, fontWeight: 700, color: '#1a1a2e' }}>FAQ Management</Typography>
					<Typography sx={{ fontSize: 12, color: '#aaa', mt: 0.3 }}>{total} total entries</Typography>
				</Box>
